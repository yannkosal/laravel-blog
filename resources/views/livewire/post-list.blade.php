    ...
    <div class="flex justify-between items-center border-b border-gray-100">
        <div class="text-gray-600">
            @if ($this->activeCategory() || $search)
                <button class="text-gray-900 text-sm font-semibold mb-3" wire:click="clearFilters()">Clear</button>
            @endif
            @if ($this->activeCategory())
                <x-badge wire:navigate href="{{ route('posts.blog', ['category' => $this->activeCategory()->slug]) }}"
        ...
        <div class="flex items-center space-x-4 font-light">
            ...
            <x-label>{{ __('blog.popular_posts') }}</x-label>
